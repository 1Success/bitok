...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        {
            // First part of help message is specific to bitokd / RPC client
            std::string strUsage = _("BitOK version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  bitokd [options]                     " + "\n" +
                  "  bitokd [options] <command> [params]  " + _("Send command to -server or bitokd") + "\n" +
                  "  bitokd [options] help                " + _("List commands") + "\n" +
                  "  bitokd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "bitok:"))
                fCommandLine = true;
...
int main(int argc, char* argv[])
{
    ...
    // Connect bitokd signal handlers
    noui_connect();
