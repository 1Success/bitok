...
#include "bitokbridge.moc"

BitOKBridge::BitOKBridge(BitOKGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
void BitOKBridge::setClientModel()
{
...
// This is just a hook, we won't really be setting the model...
void BitOKBridge::setWalletModel()
{
...
// This is just a hook, we won't really be setting the model...
void BitOKBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void BitOKBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void BitOKBridge::paste()
{
    emitPaste(QApplication::clipboard()->text());
}

// Options
void BitOKBridge::populateOptions()
{
...
// Transactions
bool BitOKBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void BitOKBridge::clearRecipients()
{
    recipients.clear();
}

bool BitOKBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    int inputTypes = -1;
    int nAnonOutputs = 0;
    int ringSizes = -1;
    ...
    {
        int inputType; // 0 BTK, 1 BitOK
        switch(rcp.txnTypeInd)
        {
            case TXT_BTK_TO_BTK:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BTK, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_BTK_TO_ANON:
                formatted.append(tr("<b>%1</b> to BITOK %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BTK, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b> BITOK, ring size %2 to BITOK %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BTK, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_BTK:
                formatted.append(tr("<b>%1</b> BITOK, ring size %2 to BTK %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BTK, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BTK, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsBitOKBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your bitok balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BTK, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void BitOKBridge::openCoinControl()
{
...
void BitOKBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void BitOKBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap BitOKBridge::listAnonOutputs()
{
...
void BitOKBridge::populateTransactionTable()
{
...
void BitOKBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void BitOKBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString BitOKBridge::transactionDetails(QString txid)
{
...
// Addresses
void BitOKBridge::populateAddressTable()
{
...
void BitOKBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void BitOKBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString BitOKBridge::newAddress(QString addressLabel, int addressType, QString address, bool send)
{
...
QString BitOKBridge::lastAddressError()
{
    QString sError;
...
QString BitOKBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void BitOKBridge::updateAddressLabel(QString address, QString label)
{
...
bool BitOKBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool BitOKBridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void BitOKBridge::appendMessages(QVariantList messages, bool reset)
{
    emitMessages(messages, reset);
}

void BitOKBridge::appendMessage(int row)
{
...
void BitOKBridge::populateMessageTable()
{
...
void BitOKBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool BitOKBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool BitOKBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString BitOKBridge::getPubKey(QString address)
{
    return addressModel->atm->pubkeyForAddress(address);
}

QString BitOKBridge::addressForPubKey(QString pubkey)
{
    return addressModel->atm->addressForPubkey(pubkey);
}

bool BitOKBridge::setPubKey(QString address, QString pubkey)
{
...
bool BitOKBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
...
QString BitOKBridge::createGroupChat(QString label)
{
...
QString BitOKBridge::joinGroupChat(QString privkey, QString label)
{
    /*
...
QVariantList BitOKBridge::inviteGroupChat(QString qsaddress, QVariantList invites, QString from)
{
...
void BitOKBridge::connectSignals()
{
...
QString BitOKBridge::translateHtmlString(QString string)
{
    int i = 0;
...
QVariantMap BitOKBridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap BitOKBridge::listLatestBlocks()
{
...
QVariantMap BitOKBridge::findBlock(QString searchID)
{
...
QVariantMap BitOKBridge::blockDetails(QString blkHash)
{
...
QVariantMap BitOKBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap BitOKBridge::txnDetails(QString blkHash, QString txnHash)
{
...
QVariantMap BitOKBridge::signMessage(QString address, QString message)
{
    QVariantMap result;
...
QVariantMap BitOKBridge::verifyMessage(QString address, QString message, QString signature)
{
    QVariantMap result;
...
QVariantMap BitOKBridge::getNewMnemonic(QString password, QString language)
{
    QVariantMap result;
...
QVariantMap BitOKBridge::importFromMnemonic(QString inMnemonic, QString inPassword, QString inLabel, bool fBip44, int64_t nCreateTime)
{
...
QVariantMap BitOKBridge::extKeyAccList() {
    QVariantMap result;
...
QVariantMap BitOKBridge::extKeyList() {
    QVariantMap result;
...
QVariantMap BitOKBridge::extKeyImport(QString inKey, QString inLabel, bool fBip44, int64_t nCreateTime)
{
    QVariantMap result;
    ...
    if (eKey58.Set58(sInKey.c_str()) == 0)
    {
        ...
        {
            result.insert("error_msg", "Import failed - Key must begin with BitOKcoin prefix.");
            return result;
        }
...
QVariantMap BitOKBridge::extKeySetDefault(QString extKeyID)
{
    QVariantMap result;
...
QVariantMap BitOKBridge::extKeySetMaster(QString extKeyID)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
    if (extKeyID.length() == 0)
...
QVariantMap BitOKBridge::extKeySetActive(QString extKeyID, QString isActive)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
