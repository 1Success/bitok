// Copyright (c) 2014 The BitOK developers
...
BitOKGUI::BitOKGUI(QWidget *parent):
    QMainWindow(parent),
    bridge(new BitOKBridge(this)),
    clientModel(0),
    walletModel(0),
    messageModel(0),
    ...
    nWeight(0)
{
    ...
    resize(1280, 720);
    setWindowTitle(tr("BitOKwallet") + " - " + tr("Client"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/bitok"));
    setWindowIcon(QIcon(":icons/bitok"));
#else
...
#ifdef Q_OS_WIN
    QFile html("C:/bitok/index.html");
#else
    QFile html("/opt/bitok/index.html");
#endif
...
void BitOKGUI::pageLoaded(bool ok)
{
...
void BitOKGUI::addJavascriptObjects()
{
    documentFrame->addToJavaScriptWindowObject("bridge", bridge);

}

void BitOKGUI::urlClicked(const QUrl & link)
{
...
void BitOKGUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/bitok"), tr("&About BitOK"), this);
    aboutAction->setToolTip(tr("Show information about BitOK"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for BitOK"));
    optionsAction->setMenuRole(QAction::PreferencesRole);
    toggleHideAction = new QAction(QIcon(":/icons/bitok"), tr("&Show / Hide"), this);
...
void BitOKGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("BitOKwallet") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/bitok_testnet"));
            setWindowIcon(QIcon(":icons/bitok_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/bitok_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("BitOKwallet client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/bitok_testnet"));
...
void BitOKGUI::setWalletModel(WalletModel *walletModel)
{
...
void BitOKGUI::setMessageModel(MessageModel *messageModel)
{
...
void BitOKGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("BitOKwallet client"));
    trayIcon->setIcon(QIcon(":/icons/bitok"));
...
#ifndef Q_OS_MAC
void BitOKGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void BitOKGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("data-title", tr("%n active connection(s) to BitOK network", "", count));
}

void BitOKGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void BitOKGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void BitOKGUI::changeEvent(QEvent *e)
{
...
void BitOKGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void BitOKGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::BTK, nFeeRequired));
...
void BitOKGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
...
void BitOKGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void BitOKGUI::optionsClicked()
{
    bridge->triggerElement("#navitems a[href=#options]", "click");
    showNormalIfMinimized();
}

void BitOKGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void BitOKGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}

void BitOKGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid BitOK address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void BitOKGUI::handleURI(QString strURI)
{
...
void BitOKGUI::setEncryptionStatus(int status)
{
...
void BitOKGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void BitOKGUI::changePassphrase()
{
...
void BitOKGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void BitOKGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BitOKGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void BitOKGUI::updateStakingIcon()
{
