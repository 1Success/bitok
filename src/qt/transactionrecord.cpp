...
QString TransactionRecord::getTypeLabel(const int &type)
{
    ...
    case RecvWithAddress:
        return BitOKGUI::tr("Received with");
    case RecvFromOther:
        return BitOKGUI::tr("Received from");
    case SendToAddress:
    case SendToOther:
        return BitOKGUI::tr("Sent to");
    case SendToSelf:
        return BitOKGUI::tr("Payment to yourself");
    case Generated:
        return BitOKGUI::tr("Mined");
    case RecvBitOK:
        return BitOKGUI::tr("Received bitok");
    case SendBitOK:
        return BitOKGUI::tr("Sent bitok");
    case Other:
        return BitOKGUI::tr("Other");
    default:
        return "";
    }
}
...
QString TransactionRecord::getTypeShort(const int &type)
{
    ...
    case TransactionRecord::RecvBitOK:
        return "input";
    case TransactionRecord::SendToAddress:
    case TransactionRecord::SendToOther:
    case TransactionRecord::SendBitOK:
        return "output";
    default:
        return "inout";
    }
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    int64_t nCredBTK, nCredBitOK;
    wtx.GetCredit(nCredBTK, nCredBitOK, true);
    int64_t nCredit = nCredBTK + nCredBitOK;
    ...
    {
        if (nNet > 0 && nCredBitOK > 0)
        {
            // -- credit
            TransactionRecord sub(hash, nTime, TransactionRecord::RecvBitOK, "", "", nNet, 0);
        ...
        {
            // -- debit
            TransactionRecord sub(hash, nTime, TransactionRecord::SendBitOK, "", "", nNet, 0);
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
        {
                ...
                && txout.IsAnonOutput())
            {
                ...
                if (wallet->HaveKey(ckidD))
                {
                    ...
                    sub.type = TransactionRecord::RecvBitOK;
    ...
    } else
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
                ...
                && txin.IsAnonInput())
            {
                ...
                {
                    fAllFromMe = false;
                    break; // display as send/recv bitok
                };
                continue;
            };
        ...
        if (fAllFromMe)
        {
            ...
            for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
            {
                    ...
                    && txout.IsAnonOutput())
                {
                    ...
                    sub.type = TransactionRecord::SendBitOK;
                    sub.address = CBitcoinAddress(ckidD).ToString();
