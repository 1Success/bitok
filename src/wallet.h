...
public:
    ...
    int64_t GetBitOKDebit(const CTxIn& txin) const;
    int64_t GetBitOKCredit(const CTxOut& txout) const;
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nBTK, int64_t& nBitOK) const
    {
        nBTK = 0;
        nBitOK = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nBitOK += GetBitOKCredit(txout);
            } else
                nBTK += GetCredit(txout);
            if (!MoneyRange(nBTK)
                || !MoneyRange(nBitOK))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
...
public:
    ...
    mutable bool fAvailableBitOKCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailableBitOKCreditCached;
    
    mutable int64_t nCredBTKCached;
    mutable int64_t nCredBitOKCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailableBitOKCreditCached = false;
    ...
    bool GetCredit(int64_t& nCredBTK, int64_t& nCredBitOK, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredBTK = nCredBitOK = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredBTKCached = 0;
            nCredBitOKCached = 0;
            pwallet->GetCredit(*this, nCredBTKCached, nCredBitOKCached);
            fCreditSplitCached = true;
        };
        
        nCredBTK = nCredBTKCached;
        nCredBitOK = nCredBitOKCached;
        return true;
    }
    ...
    int64_t GetAvailableBitOKCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailableBitOKCreditCached)
            return nAvailableBitOKCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailableBitOKCredit() : value out of range");
            };
        };
        
        nAvailableBitOKCreditCached = nCredit;
        fAvailableBitOKCreditCached = true;
        return nCredit;
    };
